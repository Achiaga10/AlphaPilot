...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("requested_start <= requested_end", name="ck_ingestion_batch_date_range"),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.CheckConstraint("version_sequence > 0", name="ck_candle_version_sequence_positive"),
    ...
    # Reusing the operational UUID is safe and keeps this set-based/idempotent in
    # normal one-time Alembic upgrade semantics.
    op.execute(
        """
        INSERT INTO daily_candle_versions (
            id, company_id, trading_day, open, high, low, close, volume,
            provider, feed, provenance_status, ingestion_batch_id,
            observed_at, version_sequence
        )
        SELECT
            dc.id, dc.company_id, dc.trading_day, dc.open, dc.high, dc.low,
            dc.close, dc.volume, 'LEGACY_UNKNOWN', 'UNKNOWN',
            'LEGACY_UNKNOWN', NULL, COALESCE(dc.updated_at, dc.created_at, now()), 1
    ...
    op.create_table(
        ...
        sa.Column("version_watermark_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("provider_expectation", sa.String(length=50), nullable=True),
        sa.Column("feed_expectation", sa.String(length=50), nullable=True),
        sa.Column("timeframe", sa.String(length=20), nullable=False),
        sa.Column("adjustment", sa.String(length=20), nullable=False),
        sa.Column("requested_start", sa.Date(), nullable=False),
        sa.Column("requested_end", sa.Date(), nullable=False),
        sa.Column("benchmark_ticker", sa.String(length=20), nullable=False),
        sa.Column("universe_identifier", sa.String(length=100), nullable=False),
        ...
        sa.Column("id", sa.UUID(), nullable=False),
        sa.CheckConstraint("requested_start <= requested_end", name="ck_dataset_snapshot_date_range"),
        sa.CheckConstraint("status IN ('DRAFT', 'FINALIZED')", name="ck_dataset_snapshot_status"),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.Column("ticker_at_snapshot", sa.String(length=20), nullable=False),
        sa.Column("company_name_at_snapshot", sa.String(length=255), nullable=False),
        sa.Column("exchange_at_snapshot", sa.String(length=20), nullable=False),
        sa.Column("sector_at_snapshot", sa.String(length=100), nullable=True),
        sa.Column("membership_source", sa.String(length=100), nullable=False),
        sa.Column("id", sa.UUID(), nullable=False),
        sa.CheckConstraint("role IN ('UNIVERSE', 'BENCHMARK')", name="ck_dataset_member_role"),
        sa.ForeignKeyConstraint(["company_id"], ["companies.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(
            ["snapshot_id"], ["research_dataset_snapshots.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["company_id"], ["companies.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(
            ["candle_version_id"], ["daily_candle_versions.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["snapshot_id"], ["research_dataset_snapshots.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("snapshot_id", "candle_version_id"),
        sa.UniqueConstraint(
            "snapshot_id", "company_id", "trading_day", name="uq_dataset_candle_company_day"
        ),
    )
    op.create_index(
    ...
    op.execute(
        ...
        FOR EACH ROW EXECUTE FUNCTION alphapilot_reject_candle_version_mutation();
        ...
        FOR EACH ROW EXECUTE FUNCTION alphapilot_protect_terminal_ingestion_batch();
        ...
        FOR EACH ROW EXECUTE FUNCTION alphapilot_protect_finalized_snapshot();
        ...
        BEGIN
            ...
            IF EXISTS (
                SELECT 1 FROM research_dataset_snapshots
                WHERE id = target_snapshot AND status = 'FINALIZED'
            ) THEN
                RAISE EXCEPTION 'Finalized research snapshot members are immutable';
            END IF;
            IF TG_OP = 'DELETE' THEN
                RETURN OLD;
            END IF;
        ...
        BEFORE INSERT OR UPDATE OR DELETE ON research_dataset_universe_members
        FOR EACH ROW EXECUTE FUNCTION alphapilot_protect_snapshot_member();

        CREATE TRIGGER trg_dataset_candle_members_immutable
        BEFORE INSERT OR UPDATE OR DELETE ON research_dataset_candle_members
        FOR EACH ROW EXECUTE FUNCTION alphapilot_protect_snapshot_member();
        """
    )
...
def downgrade() -> None:
    op.execute("DROP TRIGGER IF EXISTS trg_dataset_candle_members_immutable ON research_dataset_candle_members")
    op.execute("DROP TRIGGER IF EXISTS trg_dataset_universe_members_immutable ON research_dataset_universe_members")
    op.execute("DROP FUNCTION IF EXISTS alphapilot_protect_snapshot_member()")
    op.execute("DROP TRIGGER IF EXISTS trg_research_snapshots_finalized_immutable ON research_dataset_snapshots")
    op.execute("DROP FUNCTION IF EXISTS alphapilot_protect_finalized_snapshot()")
    op.execute("DROP TRIGGER IF EXISTS trg_ingestion_batches_terminal_immutable ON market_data_ingestion_batches")
    op.execute("DROP FUNCTION IF EXISTS alphapilot_protect_terminal_ingestion_batch()")
    op.execute("DROP TRIGGER IF EXISTS trg_daily_candle_versions_immutable ON daily_candle_versions")
    op.execute("DROP FUNCTION IF EXISTS alphapilot_reject_candle_version_mutation()")

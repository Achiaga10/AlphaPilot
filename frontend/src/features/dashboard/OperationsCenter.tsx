...
import type { NotificationPreferences, OperationalIncident } from '../../types/operations'
...
function IncidentCard({ incident, busy, onAcknowledge }: {
  ...
  return <li className={`operations-incident operations-incident--${incident.severity.toLowerCase()}`}>
    ...
    <p><strong>External notification:</strong> {label(incident.notification_state)}</p>
...
function NotificationPreferencesForm({ initial, busy, onSave }: {
  initial: NotificationPreferences
  busy: boolean
  onSave: (preferences: NotificationPreferences) => void
}) {
  const [preferences, setPreferences] = useState(initial)
  const toggle = (name: keyof NotificationPreferences) => (event: React.ChangeEvent<HTMLInputElement>) =>
    setPreferences((current) => ({ ...current, [name]: event.target.checked }))
  return <form className="notification-preferences" onSubmit={(event) => { event.preventDefault(); onSave(preferences) }}>
    <div className="notification-toggle-grid">
      <label><input type="checkbox" checked={preferences.notifications_enabled} onChange={toggle('notifications_enabled')} /> Notifications enabled</label>
      <label><input type="checkbox" checked={preferences.email_enabled} onChange={toggle('email_enabled')} /> Email alerts enabled</label>
      <label><input type="checkbox" checked={preferences.warning_enabled} onChange={toggle('warning_enabled')} /> Warning alerts</label>
      <label><input type="checkbox" checked={preferences.critical_enabled} onChange={toggle('critical_enabled')} /> Critical alerts</label>
      <label><input type="checkbox" checked={preferences.recovery_enabled} onChange={toggle('recovery_enabled')} /> Recovery alerts</label>
      <label><input type="checkbox" checked={preferences.daily_summary_enabled} onChange={toggle('daily_summary_enabled')} /> Daily Operations summary</label>
    </div>
    <label>Recipient address<input aria-label="Recipient address" type="email" value={preferences.recipient ?? ''} onChange={(event) => setPreferences((current) => ({ ...current, recipient: event.target.value || null }))} /></label>
    <button className="button button--secondary" disabled={busy} type="submit">Save notification preferences</button>
  </form>
}
...
export function OperationsCenter() {
  ...
  const notificationStatus = operations.notificationStatus.data
  const preferences = operations.notificationPreferences.data
  ...
  const busy = operations.acknowledge.isPending || operations.evaluate.isPending || operations.saveNotificationPreferences.isPending || operations.sendTestNotification.isPending
  return <section className="operations-center" aria-labelledby="operations-title">
    <div className="section-heading"><div><p className="eyebrow">DETERMINISTIC OPERATIONS &amp; DELIVERY</p><h2 id="operations-title">Operations Center</h2></div><span className={`badge ${health.overall_health === 'DEGRADED' ? 'badge--danger' : health.overall_health === 'ATTENTION' ? 'badge--warning' : 'badge--neutral'}`}>{health.overall_health}</span></div>
    <p className="forward-boundary"><strong>Observational only.</strong> Incidents and notifications cannot trade, change Micho decisions, alter Forward economics, or make News authoritative.</p>
    ...
    <div className="operations-health-grid">
      ...
      <article aria-labelledby="notification-delivery-title"><h3 id="notification-delivery-title">Notification Delivery</h3>{notificationStatus ? <dl className="config-grid"><div><dt>State</dt><dd>{notificationStatus.enabled ? 'Enabled' : 'Disabled'}</dd></div><div><dt>Email configured</dt><dd>{notificationStatus.configured ? 'YES' : 'NO'}</dd></div><div><dt>Queue / failed</dt><dd>{notificationStatus.queue_depth} / {notificationStatus.failed_count}</dd></div><div><dt>Last delivery</dt><dd>{notificationStatus.last_delivery_at ? new Date(notificationStatus.last_delivery_at).toLocaleString() : 'Never'}</dd></div></dl> : <p className="muted">Notification status unavailable.</p>}</article>
    </div>
    {preferences ? <details className="notification-settings"><summary>Notification preferences</summary><NotificationPreferencesForm initial={preferences} busy={busy} onSave={(value) => operations.saveNotificationPreferences.mutate(value)} /><div className="table-actions"><button className="button button--secondary" disabled={busy || !notificationStatus?.enabled || !notificationStatus.email_enabled || !notificationStatus.configured} onClick={() => operations.sendTestNotification.mutate()}>Send test email</button></div>{operations.sendTestNotification.isSuccess ? <p role="status">TEST notification queued through the durable delivery path.</p> : null}{operations.sendTestNotification.isError ? <p role="alert">Test notification could not be queued.</p> : null}</details> : null}
    ...
    <details><summary>Notification history ({operations.notifications.data?.length ?? 0})</summary>{operations.notifications.data?.length ? <div className="table-shell"><table><thead><tr><th>Time</th><th>Severity</th><th>Incident</th><th>Channel</th><th>Status</th><th /></tr></thead><tbody>{operations.notifications.data.map((notification) => <tr key={notification.id}><td>{new Date(notification.created_at).toLocaleString()}</td><td>{notification.priority}</td><td>{notification.subject}</td><td>{notification.channel}</td><td>{label(notification.status)}</td><td>{notification.status === 'FAILED' ? <button className="button button--secondary" disabled={operations.retryNotification.isPending} onClick={() => operations.retryNotification.mutate(notification.id)}>Retry</button> : null}</td></tr>)}</tbody></table></div> : <p className="empty-inline">No external notification history.</p>}</details>

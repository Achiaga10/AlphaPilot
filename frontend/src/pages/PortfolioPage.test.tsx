...
test('forward paper evidence is visibly separate from historical research', async () => {
  ...
  await renderReadyPortfolio()
  const summary = await screen.findByText((_, element) => element?.tagName === 'SUMMARY' && Boolean(element.textContent?.includes('Forward Paper Evidence')))
  expect(summary).toHaveTextContent(/1 open \/ 1 closed/)
  await user.click(summary)
  expect(screen.getByText(/separate from current ResearchPortfolio holdings/i)).toBeInTheDocument()
  expect(screen.getByRole('heading', { name: 'Current Open Paper Validation Trades' })).toBeInTheDocument()
  expect(screen.getByRole('heading', { name: 'Historical Closed Paper Trades' })).toBeInTheDocument()
  expect(screen.getAllByText(/125.50/).length).toBeGreaterThanOrEqual(2)
...
test('plan request is exact and response renders BUY SELL HOLD SKIP plus dates and statuses', async () => {
  ...
  expect(screen.getAllByText('Sector limit reached').length).toBeGreaterThan(0)
  ...
  await user.click(screen.getByRole('tab', { name: /Universe Evaluated/ }))

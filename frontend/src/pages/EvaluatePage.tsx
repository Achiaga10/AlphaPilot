      ...
      {plan && status ? (
        <div className="page-stack plan-results" aria-live="polite">
          ...
          <div className="analysis-strip">
            <div><span>Strategy</span><strong>{strategyLabel(plan.strategy)}</strong></div><div><span>Profile</span><strong>{plan.strategy_profile.profile_id} v{plan.strategy_profile.version}</strong></div><div><span>Selection</span><strong>{selectionLabel(plan.selection_policy)}</strong></div><div><span>Sizing</span><strong>{sizingLabel(plan.sizing_policy)}</strong></div><div><span>Requested</span><strong>{formatDate(plan.requested_as_of_date)}</strong></div><div><span>Completed analysis session</span><strong>{formatDate(plan.analysis_as_of_date)}</strong></div>
          </div>
